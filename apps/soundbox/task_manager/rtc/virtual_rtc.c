...
static int rtc_simulate_init(const struct dev_node *node, void *arg)
{
    ...
    u16 crc = CRC16((u8 *)&__this->sys_simulate_time.year, sizeof(struct sys_time) - 2);
    if (crc == __this->sys_simulate_time.crc) {
...
void set_virtual_rtc_tick(u64 actual_ns)
{
    ...
    if (cal_flag) {
        ...
        __this->sys_simulate_time.crc = CRC16((u8 *)&__this->sys_simulate_time.year, sizeof(struct sys_time) - 2);
        /*
