...
static void mic_effect_destroy(struct __mic_effect **hdl)
{
    ...
    if (effect->p_energy_hdl) {
        audio_energy_detect_close(effect->p_energy_hdl);
    }
...
*/
/*----------------------------------------------------------------------------*/
void mic_automute_handle(u8 event, u8 ch)
{
    static u8 dvol = 0xff;
    printf("mic>>>> ch:%d %s\n", ch, event ? ("MUTE") : ("UNMUTE"));
    if (ch == 1) {
        // mute: 降低DAC增益或者mic数字音量
        // 示例代码
        if (event) {
            dvol = mic_effect_get_dvol();
            if (dvol > 10) {
                mic_effect_set_dvol(10);
            }
        } else {
            if (10 == mic_effect_get_dvol() && dvol != 0xff) {
                mic_effect_set_dvol(dvol);
            }
        }
    }
}
...
bool mic_effect_start(void)
{
    ...
    if (effect->parm.effect_config & BIT(MIC_EFFECT_CONFIG_ENERGY_DETECT)) {
        mic_energy_det_param.event_handler = mic_automute_handle;
        effect->p_energy_hdl = audio_energy_detect_open(&mic_energy_det_param);
    }
    ///声音门限初始化
...
///啸叫抑制初始化
...
#if SYS_DIGVOL_GROUP_EN
        void *dvol_entry = sys_digvol_group_ch_open("mic_mic", -1, &effect_dvol_default_parm);
    ...
    if (output_channels != ch_num) {
        effect->channel_zoom = channel_switch_open(output_channels == 2 ? AUDIO_CH_LR : AUDIO_CH_DIFF, output_channels == 4 ? 2560	+ 128 : 1024);
#if TCFG_APP_RECORD_EN
        effect->channel_zoom->entry.prob_handler = prob_handler_to_record;
...
#if TCFG_APP_RECORD_EN
        if (!effect->channel_zoom) { //如果通道变换节点不开，录音取数位置放在DAC前
            effect->dac->entry.prob_handler = prob_handler_to_record;
        }
#endif//TCFG_APP_RECORD_EN
    }
    ...
    if (effect->p_energy_hdl) {
        entries[entry_cnt++] = audio_energy_detect_entry_get(effect->p_energy_hdl);
    }
    if (effect->p_noisegate_hdl) {
    ...
    if (effect->usbmic_hdl) {
        //从变采样前一节点分流
        audio_stream_add_entry(entries[entry_cnt - 3], &effect->usbmic_hdl->entry);
    }
#endif
...
void mic_effect_mic_gain_parm_fill(EFFECTS_MIC_GAIN_PARM *parm, u8 fade, u8 online)
{
    ...
    mic_stream_set_gain(__this->mic, parm->gain);
}
/*----------------------------------------------------------------------------*/
...
static const struct eq_seg_info your_audio_out_eq_tab[] = {
    {0, EQ_IIR_TYPE_HIGH_PASS, 60, (-12) *(1 << 20), (int)(0.4f * (1 << 24))},
    {1, EQ_IIR_TYPE_BAND_PASS, 150, (-1.9) *(1 << 20), (int)(0.7f * (1 << 24))},
    {2, EQ_IIR_TYPE_BAND_PASS, 300, (-0.3) *(1 << 20), (int)(0.7f * (1 << 24))},
    {3, EQ_IIR_TYPE_BAND_PASS, 500, (0.2) *(1 << 20), (int)(0.7f * (1 << 24))},
    {4, EQ_IIR_TYPE_BAND_PASS, 1000, (0.5) *(1 << 20), (int)(0.7f * (1 << 24))},
    {5, EQ_IIR_TYPE_BAND_PASS, 2000, (1) *(1 << 20), (int)(0.7f * (1 << 24))},
    {6, EQ_IIR_TYPE_BAND_PASS, 3000, (1.5) *(1 << 20), (int)(0.7f * (1 << 24))},
    {7, EQ_IIR_TYPE_BAND_PASS, 5000, (0.5) *(1 << 20), (int)(0.7f * (1 << 24))},
    {8, EQ_IIR_TYPE_BAND_PASS, 10000, (-0.5) *(1 << 20), (int)(0.7f * (1 << 24))},
    {9, EQ_IIR_TYPE_LOW_PASS, 16000, (-0.3) *(1 << 20), (int)(0.7f * (1 << 24))},
};
float your_eq_coeff_tab[10][5];

__attribute__((weak))int mic_eq_get_filter_info(void *eq, int sr, struct audio_eq_filter_info *info)
{
    log_info("mic_eq_get_filter_info fff \n");
#if 1  //使用高低音系数表
    ...
    info->L_gain = info->R_gain = 1;//总增益填写，用户可修改（-20~20db）
